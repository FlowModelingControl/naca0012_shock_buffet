...
def fetch_surface_data(path):
    ...
    x_max = data.x.max()
    x_up = data[(data.y >= 0) & (data.x < 0.999*x_max)].x
    x_up = x_up.values / x_max
    z_up = data[(data.y >= 0) & (data.x < 0.999*x_max)].z
    z_up = z_up.values / x_max
    f_up = data[(data.y >= 0) & (data.x < 0.999*x_max)].f.values
    x_low = data[(data.y < 0) & (data.x < 0.999*x_max)].x
    x_low = x_low.values / x_max
    z_low = data[(data.y < 0) & (data.x < 0.999*x_max)].z
    z_low = z_low.values / x_max
    f_low = data[(data.y < 0) & (data.x < 0.999*x_max)].f.values
...
def average_surface_data(path, file_name, t_start=0.0, t_end=1000.0):
    ...
    for i, t in enumerate(times[i_start+1:i_end]):
        _, _, f_up, _, _, f_low = fetch_surface_data(path + t + "/" + file_name)
